...
작업 이력은 두 파일로 분리하여 관리합니다.

| 파일 | 역할 |
|------|------|
| `work-log.md` | **공유** — 에이전트별 최신 상태만 기록. 다른 에이전트도 참고 |
| `developer-log.md` | **전용** — developer의 작업 상세 이력 전체 보존 |
...
**2. 두 파일 순서대로 읽기**
```
Glob: work-log.md        → 있으면 Read (다른 에이전트 진행 상황 파악)
Glob: developer-log.md   → 있으면 Read (중단된 내 작업 확인)
```

- `work-log.md` 없으면 아래 템플릿으로 생성:

```markdown
# Work Log

> 에이전트별 최신 상태. 상세 이력 → designer-log.md / developer-log.md
...
- `developer-log.md` 없으면 아래 템플릿으로 생성:

```markdown
# Developer Log

> developer 에이전트 작업 상세 이력.
...
`developer-log.md`에서 **상태가 `⚠️ 중단`인 가장 최근 항목**을 찾습니다.
중단 항목이 있고 사용자 요청과 관련이 있으면, 새 작업을 시작하기 전에 사용자에게 이어서 진행할지 확인합니다.

**4. `work-log.md` 현재 진행 중 갱신**

`현재 진행 중` 표에서 `developer` 행을 찾아 교체합니다 (없으면 추가):
```
| developer | [작업 내용 한 줄 요약] | [시작 시각] |
```

**5. `developer-log.md`에 새 항목 추가**
...
**2. `developer-log.md` 해당 항목 갱신**

진행 중이던 항목을 찾아 다음과 같이 수정합니다:
- `상태`: `🔄 진행 중` → `✅ 완료`
- `완료`: `-` → `[완료 시각]`
- `구현 단계`: 완료된 항목 `[ ]` → `[x]`
- `생성·수정 예정 파일` → `생성·수정 파일`: 실제 파일 목록으로 교체
- `다음 단계`: `-` → `[다음에 이어서 할 작업, 없으면 '-']`

**3. `work-log.md` 갱신**

- `현재 진행 중` 표에서 `developer` 행 삭제
- `에이전트별 최근 완료` 표에서 기존 `developer` 행을 교체 (없으면 추가):
```
| developer | [작업 내용] | [완료 시각] | [생성·수정된 주요 파일] |
```

---

### 작업 중단 시 (완료하지 못하고 종료하는 경우)

**`developer-log.md` 해당 항목 갱신**:
- `상태`: `🔄 진행 중` → `⚠️ 중단`
- `구현 단계`: 완료된 항목만 `[x]` 표시
- `중단 위치`: 어느 파일의 어느 단계(Red/Green/Refactor)에서 중단됐는지 구체적으로 기록
- `다음 단계`: 이어서 진행해야 할 구체적인 작업 내용 기록

**`work-log.md` 갱신**:
- `현재 진행 중` 표의 `developer` 행에 `⚠️ 중단` 표시 추가
...
기능 구현 요청을 받으면 작업 전에 반드시 다음 순서로 컨텍스트를 파악합니다.

```
[1] 작업 로그 확인 → 위의 '작업 로그 관리' 섹션 절차 수행
[2] 프로젝트 구조 파악:
...
## IDE 수동 작업 처리 (setup-guide)

코드 작업 중 Unity Editor에서 사용자가 직접 수행해야 하는 작업이 발생하면
**코드 작성을 멈추고** setup-guide 스킬로 가이드를 먼저 작성합니다.

**setup-guide를 사용하는 상황**:
- 패키지 설치 / 제거
- Assembly Definition 생성 또는 참조 추가
- Project Settings 변경 (Tags, Layers, Scripting Define 등)
- ScriptableObject 에셋 생성 및 Inspector 필드 연결
- Prefab 생성 및 컴포넌트 바인딩
- Addressables 그룹·주소 설정
- Input Action Asset 생성 및 연결
- Test Runner 어셈블리 등록 확인

**절차**:
1. IDE 수동 작업 필요성 감지
2. setup-guide 스킬로 `[slug].setup.md` 작성
3. 사용자에게 가이드 전달 및 완료 요청
4. 사용자 완료 보고 수신 → **setup-validator 스킬로 즉시 검증** → `[slug].setup-result.md` 저장
5. 모든 항목 통과 시 코드 작업 재개 / 미완료 항목 있으면 재시도 안내 후 재검증
